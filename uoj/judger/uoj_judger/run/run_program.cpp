...
void append_extra_readable_files(const string& dir_path)
{
    const char *curdir =".";
    const char *predir ="..";
    ...
    std::queue<DIR*> dir_q;
    std::queue<std::string> dirname_q;
    dir_q.push(dir);
    dirname_q.push(dirname);
    while (!dir_q.empty() && !dirname_q.empty()) {
        dir = dir_q.front();
        dirname = dirname_q.front();
        dir_q.pop();
        dirname_q.pop();
        while ((entry = readdir(dir)) != NULL) {
            if (entry->d_type == DT_DIR) {
                if (strcmp(entry->d_name, curdir) && strcmp(entry->d_name, predir)) {
                    std::string sub_dirname = dirname + "/" + entry->d_name;
                    DIR* sub_dir;
                    if ((sub_dir = opendir(sub_dirname.c_str()))== NULL) {
                        fprintf(stderr, "opendir(): %s\n", sub_dirname.c_str());
                    } else {
                        dir_q.push(sub_dir);
                        dirname_q.push(sub_dirname);
                    }
                    run_program_config.extra_readable_files.push_back(sub_dirname + "/*");
               }
           }
        }
    }
...
void parse_args(int argc, char **argv) {
	...
	if (run_program_config.type == "python2") {
		...
		run_program_config.argv.insert(run_program_config.argv.begin(), pre, pre + 3);
	} else if (run_program_config.type == "Lua") {
		string pre[1] = {"/usr/bin/lua"};
		run_program_config.argv.insert(run_program_config.argv.begin(), pre, pre + 1);
		// all source files must be readable
		append_extra_readable_files(run_program_config.work_path);
	}
}
