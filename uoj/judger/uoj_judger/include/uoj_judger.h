...
string file_preview(const string &name, const size_t &len = 20480) {
...
RunCompilerResult compile_lua(const string &name, const string &path = work_path) {
	return run_compiler(path.c_str(),
	            "/usr/bin/luac" , "-o", name.c_str(), (name + ".code").c_str(), NULL);
}
...
RunCompilerResult compile_lua_with_implementer(const string &name, const string &path = work_path) {
	return run_compiler(path.c_str(),
	        "/usr/bin/luac" , "-o", name.c_str(), "implementer.lua", NULL);
}
/*
RunCompilerResult compile_python2(const string &name, const string &path = work_path) {
	return run_compiler(path.c_str(),
...
RunCompilerResult compile_with_implementer(const string& name)  {
	...
	if (lang == "Lua") {
        //require() can only load .lua files
        executef("mv %s/%s %s/%s", work_path.c_str(), (name + ".code").c_str(), work_path.c_str(), (name + ".lua").c_str());
        return compile_lua_with_implementer(name);
	}
...
RunCompilerResult compile_scene_queustion(const string& name) {
    //fill answer code
    executef("bash %s/fill_answer.sh %s/%s %s", work_path.c_str(), work_path.c_str(), (name + ".code").c_str(), work_path.c_str());
    executef("mv %s/Main.lua %s/implementer.lua", work_path.c_str(), work_path.c_str());
    return compile_lua_with_implementer(name);
}
...
CustomTestInfo ordinary_custom_test(const string &name) {
	...
	return CustomTestInfo(pro_ret.ust, pro_ret.usm, 
			info, exp, file_preview(output_file_name, 20480));
}
